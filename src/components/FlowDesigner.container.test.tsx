...
describe('<FlowDesigner /> renders correctly', () => {
  it('<FlowDesigner /> renders correctly', () => {
    ...
    const ports = new Map();
    const links = new Map();
    const tree = renderer.create(<FlowDesigner startMoveNodeTo={noOp} moveNodeTo={noOp} moveNodeToEnd={noOp} setNodeTypes={noOp} nodes={nodes} ports={ports} links={links} grid={mockGrid} reduxMountPoint="mountPoint">
