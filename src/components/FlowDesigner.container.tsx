...
export class FlowDesigner extends React.Component {
    ...
    if (children) {
      children.forEach(element => {
        switch (element.type.displayName) {
          case 'NodeType':
            nodeTypeMap = { ...nodeTypeMap, [element.props.type]: {
                component: element.props.component
              }};
            break;
          case 'LinkType':
            linkTypeMap = { ...linkTypeMap, [element.props.type]: {
                component: element.props.component
              }};
            break;
          case 'PortType':
            portTypeMap = { ...portTypeMap, [element.props.type]: {
                component: element.props.component
              }};
            break;
          default:
            invariant(false, `<${element.type.displayName} /> is an unknown component configuration`);
  ...
  render() {
	...
	<svg onClick={this.props.onClick} ref={c => this.node = c} width="100%">
		...
		<ZoomHandler transform={this.props.transform} transformToApply={this.props.transformToApply} setZoom={this.props.setZoom}>
			<Grid gridComponent={this.props.gridComponent} />
			<g>
				<NodesRenderer nodeTypeMap={this.state.nodeTypeMap} startMoveNodeTo={this.props.startMoveNodeTo} moveNodeTo={this.props.moveNodeTo} moveNodeToEnd={this.props.moveNodeToEnd} nodes={this.props.nodes} snapToGrid={this.props.snapToGrid} />
				<PortsRenderer portTypeMap={this.state.portTypeMap} ports={this.props.ports} />
				<LinksRenderer linkTypeMap={this.state.linkTypeMap} links={this.props.links} ports={this.props.ports} />
			</g>
		</ZoomHandler>
	</svg>
);
