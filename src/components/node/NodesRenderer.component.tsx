...
class NodesRenderer extends React.Component {
  ...
  renderNode(node) {
    ...
    if (!ConcreteComponent) {
      invariant(false, `<NodesRenderer />  the defined node type in your graph model hasn't been mapped into
				the dataflow configuration, check NodeType documentation`);
    }
    return <ConcreteComponent node={node} startMoveNodeTo={this.props.startMoveNodeTo} moveNodeTo={this.props.moveNodeTo} moveNodeToEnd={this.props.moveNodeToEnd} key={node.id} snapToGrid={this.props.snapToGrid} />;
  }
