...
export function isTopologyElseThrow(topology) {
  ...
  throwInDev(`Should be a topology 'OUTGOING' or 'INCOMING', was given ${topology && topology.toString()}`);
  return false;
}
...
export const setComponentType = curry((componentType, port) => {
  ...
  throwInDev(`componentType should be a string, was given ${componentType && componentType.toString()}`);
  return port;
});
...
export function isWhiteListAttribute(key) {
  ...
  throwInDev(`${key} is a protected value of the Port, please use get${upperFirst(key)} set${upperFirst(key)} from this module to make change on those values`);
  return false;
}
 ...
 * @returns {PortRecord}
 */
export const create = curry((id, nodeId, index, topology, componentType) => flow([setId(id), setNodeId(nodeId), setIndex(index), setTopology(topology), setComponentType(componentType)])(new PortRecord()));