...
export function isWhiteListAttribute(key) {
  ...
  throwInDev(`${key} is a protected value of the Node, please use get${upperFirst(key)} set${upperFirst(key)} from this module to make change on those values`);
  return false;
}
 ...
 * @returns {NodeRecord}
 */
export const create = curry((id, position, size, componentType) => flow([setId(id), setPosition(position), setSize(size), setComponentType(componentType)])(new NodeRecord()));