...
import { FLOWDESIGNER_FLOW_ADD_ELEMENTS, FLOWDESIGNER_FLOW_RESET, FLOWDESIGNER_FLOW_LOAD, FLOWDESIGNER_FLOW_SET_ZOOM, FLOWDESIGNER_PAN_TO, FLOWDESIGNER_NODETYPE_SET } from '../constants/flowdesigner.constants';
...
function combinedReducer(state = defaultState, action) {
  return [nodesReducer, linksReducer, portsReducer, nodeTypeReducer].reduce((cumulatedState, subReducer) => subReducer(cumulatedState, action), state);
}

export function reducer(state, action) {
  switch (action.type) {
    case FLOWDESIGNER_FLOW_ADD_ELEMENTS:
      return action.listOfActionCreation.reduce((cumulativeState, actionCreation) => combinedReducer(cumulativeState, actionCreation), state);
    case FLOWDESIGNER_FLOW_RESET:
      return defaultState.set('nodeTypes', state.get('nodeTypes'));
    case FLOWDESIGNER_FLOW_LOAD:
      return action.listOfActionCreation.reduce((cumulativeState, actionCreation) => combinedReducer(cumulativeState, actionCreation), defaultState.set('nodeTypes', state.get('nodeTypes')));
    case FLOWDESIGNER_FLOW_SET_ZOOM:
      return state.set('transform', action.transform);
    case FLOWDESIGNER_PAN_TO:
      return state.update('transformToApply', () => zoomIdentity.translate(state.get('transform').x, state.get('transform').y).scale(state.get('transform').k).scale(1 / state.get('transform').k).translate(-(state.get('transform').x + action.x), -(state.get('transform').y + action.y)));
    default:
...
export function calculatePortsPosition(state, action) {
  ...
  if ((/FLOWDESIGNER_NODE_/.exec(action.type) && action.type !== 'FLOWDESIGNER_NODE_REMOVE') || (/FLOWDESIGNER_PORT_/.exec(action.type) && action.type !== 'FLOWDESIGNER_PORT_REMOVE') || /FLOWDESIGNER.FLOW_/.exec(action.type) || action.type === FLOWDESIGNER_NODETYPE_SET) {
    if (action.nodeId) {
