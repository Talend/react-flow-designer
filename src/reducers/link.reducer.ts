...
import { FLOWDESIGNER_LINK_ADD, FLOWDESIGNER_LINK_SET_TARGET, FLOWDESIGNER_LINK_SET_SOURCE, FLOWDESIGNER_LINK_REMOVE, FLOWDESIGNER_LINK_SET_GRAPHICAL_ATTRIBUTES, FLOWDESIGNER_LINK_REMOVE_GRAPHICAL_ATTRIBUTES, FLOWDESIGNER_LINK_SET_DATA, FLOWDESIGNER_LINK_REMOVE_DATA } from '../constants/flowdesigner.constants';
  ...
  switch (action.type) {
    case FLOWDESIGNER_LINK_ADD:
      ...
      if (!state.getIn(['ports', action.targetId])) {
        invariant(false, `can't set a non existing target with id ${action.targetId} on link ${action.linkId}`);
      }

      if (!state.getIn(['ports', action.sourceId])) {
        invariant(false, `can't set a non existing source with id ${action.sourceId} on link ${action.linkId}`);
      }
      ...
      return state.setIn(['links', action.linkId], new LinkRecord({
        ...
        data: new LinkData(action.data).set('properties', fromJS(action.data && action.data.properties) || new Map()),
        graphicalAttributes: new LinkGraphicalAttributes(action.graphicalAttributes).set('properties', fromJS(action.graphicalAttributes && action.graphicalAttributes.properties) || new Map())
    ...
    case FLOWDESIGNER_LINK_SET_TARGET:
      if (!state.getIn(['links', action.linkId])) {
        invariant(false, `can't set a target ${action.targetId} on non existing link with id ${action.linkId}`);
      }
    ...
    case FLOWDESIGNER_LINK_SET_SOURCE:
      if (!state.getIn(['links', action.linkId])) {
        invariant(false, `can't set a source ${action.sourceId} on non existing link with id ${action.linkId}`);
      }

      if (!state.getIn(['ports', action.sourceId])) {
        invariant(false, `can't set a non existing target with id ${action.sourceId} on link ${action.linkId}`);
      }
