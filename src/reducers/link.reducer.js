...
import { Flow, Link } from '../api';

const defaultState = new Map();

function addLinkDeprecated(action, state) {
...
function addLink(state, action) {
	if (action.link && Link.isLink(action.link)) {
		return Flow.addLink(state, action.link);
	}
	// @deprecated bellow
	return addLinkDeprecated(action, state);
}
