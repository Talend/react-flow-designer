...
describe('check linkreducer', () => {
  const initialState = defaultState.set('nodes', new Map().set('id1', new NodeRecord({
    id: 'id1'
  })).set('id2', new NodeRecord({
    id: 'id2'
  })).set('id3', new NodeRecord({
    id: 'id3'
  }))).set('links', new Map().set('id1', new LinkRecord({
    id: 'id1',
    sourceId: 'id1',
    targetId: 'id2',
    data: new Map().set('attr', 'attr'),
    graphicalAttributes: new Map().set('properties', fromJS({ attr: 'attr' }))
  }))).set('ports', new Map().set('id1', new PortRecord({
    id: 'id1',
    nodeId: 'id1'
    ...
    id: 'id6',
    nodeID: 'id3'
  }))).set('parents', new Map().set('id1', new Map()).set('id2', new Map().set('id1', 'id1')).set('id3', new Map())).set('childrens', new Map().set('id1', new Map().set('id2', 'id2')).set('id2', new Map()).set('id3', new Map()));
  ...
  it('FLOWDESIGNER_LINK_REMOVE should remove link from state', () => {
    expect(linkReducer(initialState, { type: 'FLOWDESIGNER_LINK_REMOVE', linkId: 'id1' })).toMatchSnapshot();
  });
