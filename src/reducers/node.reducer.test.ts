...
import { FLOWDESIGNER_NODE_SET_TYPE, FLOWDESIGNER_NODE_MOVE_START, FLOWDESIGNER_NODE_MOVE, FLOWDESIGNER_NODE_MOVE_END } from '../constants/flowdesigner.constants';
...
describe('Check node reducer', () => {
  ...
  it('Create a startPosition when receiving a FLOWDESIGNER_NODE_MOVE_START command', () => {
    ...
    }).getIn(['nodes', 'id1', 'graphicalAttributes', 'properties', 'startPosition']).toJS()).toEqual({ x: 50, y: 50 });
  });
  ...
  it('empty the startPostion when receiving a FLOW_DESIGNER_MOVE_END command', () => {
    expect(nodeReducer(initialState, {
      ...
      nodeId: 'id1',
      nodePosition: { x: 50, y: 50 }
    }).getIn(['nodes', 'id1', 'graphicalAttributes', 'properties', 'startPosition'])).toEqual(undefined);
  });
