...
import { FLOWDESIGNER_NODE_ADD, FLOWDESIGNER_NODE_MOVE_START, FLOWDESIGNER_NODE_MOVE, FLOWDESIGNER_NODE_APPLY_MOVEMENT, FLOWDESIGNER_NODE_MOVE_END, FLOWDESIGNER_NODE_SET_TYPE, FLOWDESIGNER_NODE_SET_GRAPHICAL_ATTRIBUTES, FLOWDESIGNER_NODE_REMOVE_GRAPHICAL_ATTRIBUTES, FLOWDESIGNER_NODE_SET_DATA, FLOWDESIGNER_NODE_REMOVE_DATA, FLOWDESIGNER_NODE_SET_SIZE, FLOWDESIGNER_NODE_REMOVE, FLOWDESIGNER_NODE_UPDATE } from '../constants/flowdesigner.constants';
import { Node } from '../api';
import { NodeRecord, PositionRecord, SizeRecord, NodeGraphicalAttributes } from '../constants/flowdesigner.model';
  ...
  switch (action.type) {
    case FLOWDESIGNER_NODE_ADD:
      ...
      return state.setIn(['nodes', action.nodeId], new NodeRecord({
        ...
        data: new Immutable.Map(action.data).set('properties', fromJS(action.data && action.data.properties) || new Map()),
        graphicalAttributes: new NodeGraphicalAttributes(fromJS(action.graphicalAttributes)).set('nodeSize', new SizeRecord(action.graphicalAttributes.nodeSize)).set('position', new PositionRecord(action.graphicalAttributes.position)).set('properties', fromJS(action.graphicalAttributes.properties) || new Map())
      })).setIn(['out', action.nodeId], new Map()).setIn(['in', action.nodeId], new Map()).setIn(['childrens', action.nodeId], new Map()).setIn(['parents', action.nodeId], new Map());
    case FLOWDESIGNER_NODE_UPDATE:
      ...
      return state.setIn(['nodes', Node.getId(action.node)], action.node).deleteIn(['nodes', action.nodeId]).setIn(['out', Node.getId(action.node)], new Map()).setIn(['in', Node.getId(action.node)], new Map()).setIn(['childrens', Node.getId(action.node)], new Map()).setIn(['parents', Node.getId(action.node)], new Map());
    case FLOWDESIGNER_NODE_MOVE_START:
      if (!state.getIn('nodes', action.nodeId)) {
        invariant(false, `Can't move node ${action.nodeId} since it doesn't exist`);
      }

      return state.setIn(['nodes', action.nodeId, 'graphicalAttributes', 'properties', 'startPosition'], new PositionRecord(action.nodePosition));
    case FLOWDESIGNER_NODE_MOVE:
      if (!state.getIn('nodes', action.nodeId)) {
        invariant(false, `Can't move node ${action.nodeId} since it doesn't exist`);
      }

      return state.setIn(['nodes', action.nodeId, 'graphicalAttributes', 'position'], new PositionRecord(action.nodePosition));
    case FLOWDESIGNER_NODE_MOVE_END:
      if (!state.getIn('nodes', action.nodeId)) {
        invariant(false, `Can't move node ${action.nodeId} since it doesn't exist`);
      }

      return state.setIn(['nodes', action.nodeId, 'graphicalAttributes', 'position'], new PositionRecord(action.nodePosition)).deleteIn(['nodes', action.nodeId, 'graphicalAttributes', 'properties', 'startPosition']);
    ...
    case FLOWDESIGNER_NODE_SET_SIZE:
      ...
      return state.setIn(['nodes', action.nodeId, 'graphicalAttributes', 'nodeSize'], new SizeRecord(action.nodeSize));
    ...
    case FLOWDESIGNER_NODE_REMOVE:
      ...
      return inPort(state, action.nodeId).reduce((cumulativeState, port, key) => portReducer(cumulativeState, removePort(key)), outPort(state, action.nodeId).reduce((cumulativeState, port, key) => portReducer(cumulativeState, removePort(key)), state)).deleteIn(['nodes', action.nodeId]).deleteIn(['out', action.nodeId]).deleteIn(['in', action.nodeId]).deleteIn(['childrens', action.nodeId]).deleteIn(['parents', action.nodeId]);
    default:
      return state;
